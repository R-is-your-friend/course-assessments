...
[Download starter .qmd file](../student/pa7-pivot.qmd)

[Download data -- `SPIRI-Milex-data-1949-2024.xlsx`](../../data/07-SIPRI-Milex-data-1949-2024.xlsx)
:::
...
+ `na`: specify data values to interpret as missing values
:::

1. Modify the code below (potentially including the file path) to read the military expenditures data into your global environment.
...
military <- read_xlsx("../../data/07-SIPRI-Milex-data-1949-2024.xlsx", 
...
2. In addition to `NA`s, missing values were coded in two other ways in this data. Find these two methods. Edit your code in Q1, including the `na` argument to `read_xlsx()` so that these values are automatically coded as `NA`s when the data is read in.
...
Catching this when reading in the data saved us a lot of work later on!
...
3. In one pipeline, drop those columns and that row. Save this dataset as a new object named `military_clean`.
...
**4a. Use `pull()` to create a vector called `cont_region_names` with the names provided in the `cont_region` data below.**

```{r regional-totals}
cont_region <- read_xlsx("../../data/07-SIPRI-Milex-data-1949-2024.xlsx", 
...
**4b. Remove the rows of `military_clean` representing regions/continents using the `cont_region_names` vector you created. Save these changes into an updated version of `military_clean`.**
...
5. How many *countries* have no spending information in this dataset? What are some of these countries (a list to choose from will be given in Canvas)?
:::

::: callout-tip
Useful functions: `filter()`, `if_all()`, `is.na()`. You will also want to double check that any observations that are missing information aren't just a region/continent!
:::
...
#| fig-alt: "Desired line plot. The x axis has the years 1990 through 2023 and is labelled with Year. There are four lines of different colors for four countries : Algeria, Libya, Morocco, and Tunisia with a legend indicating the colors labeled as Country. The y-axis is labeled with percentages 0% through 40%. There is a title Military Expenditures in North Africa and subtitle Spending (as % of Government Spending)."
#| out-width: 70%
knitr::include_graphics("../_images/07-military-expenditure-ex-line.png")
```
...
9. Create a line plot to explore the military spending of Eastern European countries over time.
...
10. Looking at the plots you created above, which Eastern European country had the *smallest* variability in military expenditures over time? Consider a measure of variability that is *robust* to outliers (like IQR).


11. Looking at the plots you created above, you can see that for one Eastern European country, their military spending (%) over doubled and then returned to previous levels by around 2015. What is the name of the war that would explain this trend?.
:::
