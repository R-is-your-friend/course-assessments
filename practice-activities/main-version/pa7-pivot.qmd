...
**4. Change these columns to a numeric data type using `across()`. Save these changes into an updated version of `military_clean`.**
...
**5a. Use `pull()` to create a vector called `cont_region_names` with the names provided in the `cont_region` data below.**
...
**5b. Remove the rows of `military_clean` representing regions/continents using the `cont_region_names` vector you created. Save these changes into an updated version of `military_clean`.**
...
12. Looking at the plots you created above, you can see that for one Eastern European country, their military spending (%) over doubled and then returned to previous levels by around 2015. What is the name of the war that would explain this trend?.
:::
